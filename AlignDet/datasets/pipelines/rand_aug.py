...
@PIPELINES.register_module()
class SelectTopKProposals(Sequential):
    def __init__(self, topk=-1, random=False):
        self.topk = topk
        self.random = random

    def __call__(self, results):
        num_gts = len(results['gt_labels'])
        if 0 < self.topk < num_gts:
            idx = torch.randperm(num_gts) if self.random else torch.arange(num_gts)
            results['gt_bboxes'] = results['gt_bboxes'][idx][:self.topk]
            results['gt_labels'] = results['gt_labels'][idx][:self.topk]
